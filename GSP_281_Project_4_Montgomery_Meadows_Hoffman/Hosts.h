...
class Hosts : public BaseAIEntity
{
public:
	float StartTime;
	float EndTime;
	Payment *P;
	StateMachine<Hosts>* m_pStateMachine;
	Hosts(Vector2 pos, float mass, float radius, const int id, STATETYPE t):BaseAIEntity(id)
	{	
		StartTime = 0.0f;
		EndTime = 0.0f;
		m_pStateMachine = new StateMachine<Hosts>(this);
		if(t == WORKING)
	...
	inline StateMachine<Hosts>* GetFSM(void)const{return m_pStateMachine;}
